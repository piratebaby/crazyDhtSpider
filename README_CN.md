...
1. **默认完整模式**（false, true, false）：本地执行下载，同时运行DHT爬虫，处理本地和远程转发过来的metadata下载请求
2. **远程转发模式**（true, 任意, false）：所有下载metadata的任务转发到远程服务器，本地只执行DHT爬虫
3. **纯爬虫模式**（false, false, false）：只执行DHT爬虫，不处理任何下载请求，通常不使用
4. **专用下载服务器模式**（任意, 任意, true）：只处理来自其他服务器的下载请求，不执行本地DHT爬虫

### dht\_server/config.php
